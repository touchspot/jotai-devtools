...
// src/utils/useAtomsSnapshot.ts
var isEqualAtomsValues = (left, right) =>
  left.size === right.size &&
  Array.from(left).every(([left2, v]) => Object.is(right.get(left2), v));
var isEqualAtomsDependents = (left, right) =>
  left.size === right.size &&
...
function useAtomsSnapshot({ shouldShowPrivateAtoms = false, ...options } = {}) {
  const store = useDevToolsStore(options);
  const [atomsSnapshot, setAtomsSnapshot] = _react.useState.call(
    void 0,
    () => ({
...
var stateToPrintable = ([store, atoms]) =>
  Object.fromEntries(
    atoms.flatMap((atom) => {
...
var useAtomsDebugValue = (options) => {
  const enabled = _nullishCoalesce(
    _optionalChain([options, 'optionalAccess', (_2) => _2.enabled]),
            ...
            if (message.type === 'ACTION' && message.payload) {
              ...
              const computedStates =
                _optionalChain([
                  ...
                  (_17) => _17.payload,
                  'access',
                  (_18) => _18.nextLiftedState,
                  'optionalAccess',
                  (_19) => _19.computedStates,
                ]) || [];
                ...
                switch (
                  ...
                  case 'COMMIT':
                    _optionalChain([
                      ...
                      (_29) => _29.init,
                      'call',
                      (_30) => _30(getDevtoolsState(getSnapshotAt())),
                    ]);
                    snapshots.current = [];
                    break;
                  case 'JUMP_TO_ACTION':
